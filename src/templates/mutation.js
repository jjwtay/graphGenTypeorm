...
import {getColumns, getRelColumns, getType} from './model'
...
export const fieldType = (field, create = true) =>
    `${field.isList ? '[' : ''}${create ? 'Create' : 'Update'}${field.type}Input${field.isList ? ']' : ''}`
/**
 * @function
 * @param {Field} field
 * @return {boolean}
...
${Object.keys(getColumns(types[name]))
    ...
    .map(fieldKey => `    ${fieldKey}: ${types[name].fields[fieldKey].type}`).join('\n')
}
